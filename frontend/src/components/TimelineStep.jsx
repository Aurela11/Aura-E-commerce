...
const TimelineStep = ({step, isCompleted,order, isCurrent,isLastStep, icon, description}) => {
    ...
    const descriptionTextColor = isCompleted || isCurrent ? 'text-gray-900' : 'text-gray-400';
  return (
    <li className='relative b-6 sm:mb-0 sm:pl-10'>
        <div className='flex items-center'>
            <div className={`z-10 flex items-center justify-center w-6 h-6 ${
              step?.status ==='completed' ? 'bg-green-900 text-green-100' : 
              step?.status === 'pending' ? 'bg-red-700 text-red-100': 
              step?.status === 'processing' ? 'bg-blue-600 text-blue-100' :
