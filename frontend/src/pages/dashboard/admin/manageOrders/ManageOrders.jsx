...
const ManageOrders = () => {
  ...
  return (
    <div className='section__container p-6'>
      <h2 className='text-2xl font-semibold mb-4'>Manage Order</h2>
            ...
            orders && orders.map((order, index) => (
              <tr key={index}>
                ...
                <td className='py-3 px-4 border-b'>
                  <span className={`inline-block px-3 text-xs py-1 text-white
                    rounded-full ${getStatusColor(order?.status)}`}>{order?.status}</span>
                </td>
